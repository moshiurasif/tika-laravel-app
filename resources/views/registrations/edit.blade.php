    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4">
                <form action="{{route('registration.update', $category->id)}}" method="POST">
                    @csrf
                    @method('PUT')

                    <p class="mb-6">
                        <label for="name" class="w-full text-gray-600 text-sm uppercase">Name</label>
                        <input type="text" name="name" id="name" class="w-full border p-3" value="{{$user->name}}">
                    ...
                    <p class="mb-6">
                        <label for="min_age" class="w-full text-gray-600 text-sm uppercase">Date of Birth</label>
                        <input type="number" name="min_age" id="min_age" class="w-full border p-3" value="{{$user->dob}}">
                        @error('min_age')
                            <span class="block text-red-600">{{$message}}</span>
                        @enderror
                    </p>
